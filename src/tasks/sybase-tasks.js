...
const stopSybase = function(cb) {
    ...
    shell.run('docker stop sybase-trinity || true', cb);
}

const removeSybase = function(cb) {
    log.title("Removing SyBase container...");
    shell.run('docker rm sybase-trinity || true', cb);
}
...
const startSybase = function(cb) {
    ...
    async.series([
        function(callback) {
            shell.run('docker run -i --name sybase-trinity -p 5000:5000 -h dksybase -d ifnazar/sybase_15_7 bash /sybase/start', callback);
        },
        function(callback) {
...
const createLPortal = function(scriptPath, cb) {
    ...
    async.series([
        function(callback) {
            shell.run('docker cp ' + scriptPath + ' sybase-trinity:' + sqlFile, callback);
        },
        function(callback) {
            shell.run('docker exec -i sybase-trinity bash /sybase/isql "-i' + sqlFile + '"', callback);
        }
    ], cb);
